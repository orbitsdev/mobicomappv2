...
class ChapterController extends GetxController {
  ...
  EitherModel<String?> fetchChapter(BuildContext context) async {
    try {
      var response = await http.get(Uri.parse(Api.chapters_production));
      var responseData = jsonDecode(response.body);

      if (responseData['success']) {
        var data = responseData['data']['data']
            as List<dynamic>; // Extracting the chapters array from the response
        List<Chapter> new_data = (data
            .map((e) => Chapter.fromMap(e as Map<String, dynamic>))).toList();
        chapters(new_data);
  ...
  EitherModel<String?> fetchChapterLessons(
      BuildContext context, chapterId) async {
    isLessonLoading(true);
    try {
      var response = await http.get(Uri.parse(
          Api.chapter_lessons_production + "?chapter_id=${chapterId}"));
      var responseData = jsonDecode(response.body);

      if (responseData['success']) {
        var data = responseData['data']['data']
            as List<dynamic>; // Extracting the chapters array from the response
        List<Lesson> new_data = (data
            .map((e) => Lesson.fromMap(e as Map<String, dynamic>))).toList();
        lessons(new_data);
